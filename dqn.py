...
class QNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.network = nn.Sequential(
            nn.Linear(np.array(env.single_observation_space.shape).prod(), 240),
            nn.ReLU(),
            nn.Linear(240, 168),
            nn.ReLU(),
            nn.Linear(168, env.single_action_space.n),
        )
...
"""
    ...
    writer = SummaryWriter(f"../../LOGS/NextStationLondon/{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % (
    ...
    torch.save(
        ...
        f"checkpoints/DQN_{args.exp_name}_{args.seed}_{int(time.time())}.pth"
    )
