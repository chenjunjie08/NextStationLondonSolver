    ...
    def step(self, action):
        ...
        if begin in self.possible_move.keys() and end in self.possible_move[begin]:
            ...
            if self.goals_score[1] == 0:
                if self.goals_used[1] == 1 and self.dsts_total[self.game.nodes[end].dst] == 0:
                    reward += 10 / 13
            ...
            total_score = self.game.score_total(
                self.river_score, self.dst_score, self.trt_score, self.inter_score, self.goals_score, goals_used=np.zeros(5))
